...
(defun input-key-handler (game render-world dt)
  (let ((sync-position nil)
    ...
    (pill-stop (ghostie::level-main-actor (ghostie::world-level game-world)))
    (when (key= glfw:+key-left+)
      (pill-run (ghostie::level-main-actor (ghostie::world-level game-world)) -200.4))
    (when (key= glfw:+key-right+)
      (pill-run (ghostie::level-main-actor (ghostie::world-level game-world)) 200.4))
    (when (key= glfw:+key-up+)
      (let ((leftp (key= glfw:+key-left+))
            (rightp (key= glfw:+key-right+)))
        (pill-jump (ghostie::level-main-actor (ghostie::world-level game-world))
                   :y 320d0
                   :x (cond (leftp -200d0)
                            (rightp 200d0)
                            (t 0d0)))))
