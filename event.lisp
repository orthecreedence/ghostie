...
(defun dispatch-event (event)
  ...
  (let* ((event-type (getf event :type))
    ...
    (dolist (binding (event-bindings-bindings *event-bindings*))
...
(defun find-binding (event-type binding)
  ...
  (let ((find-fn (if (functionp binding)
    ...
    (find-if (lambda (bind)
             ...
             (event-bindings-bindings *event-bindings*))))
...
(defun bind-event (event types/args fn &key binding-name)
  ...
  (when binding-name
    (setf (event-bindings-bindings *event-bindings*)
...
(defun unbind-all (&optional event-type)
  ...
  (if event-type
      ...
      (setf (event-bindings-bindings *event-bindings*) nil)))
