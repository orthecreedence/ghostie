    ...
    (let ((feet-shape-c (cpw:base-c (actor-feet actor)))
      ...
      (cffi:with-foreign-object (query 'clipmunk:segment-query-info)
        (let ((body-x (cp-a:body-p-x (cp-a:shape-body feet-shape-c)))
          ...
          (cp:shape-segment-query feet-shape-c body-x body-y body-x (- 10 body-y) query)
          )
        (unless (cffi:null-pointer-p (cp-a:segment-query-info-shape query))
          (dbg :debug "Hit ground: ~s~%" (list (cp-a:segment-query-info-n-x query)
